...
1. OBSERVE: Carefully examine the image. Describe the visible symptoms: color changes, lesion patterns, texture, location on the plant (leaves, stem, fruit, roots), spread pattern, and any visible signs of the pathogen itself (mold, spores, bacterial ooze).

2. IDENTIFY THE CROP: Determine what plant/crop is shown in the image. If unclear, state your best guess and note the uncertainty.

3. DIAGNOSE: Based on the observed symptoms and the identified crop, propose 1 to 3 candidate pathogens ranked by likelihood. For each candidate, explain WHY the symptoms match that pathogen.

4. CROSS-REFERENCE MVP DATABASE: Check if any candidate matches a pathogen in the MVP database below. Matching is at GENUS level: if your candidate is "Xanthomonas citri subsp. citri" and the MVP has "Xanthomonas campestris", that is a match. If a match exists, set the candidate's mvp_match_name to the EXACT string from the MVP database. Otherwise set it to null.

5. CHECK BIOSECURITY ALERTS: Check if any candidate matches a Critical Alert Pathogen listed below. If so, set biosecurity_alert to the corresponding alert ID. These pathogens require immediate action — they override normal AMP recommendations.
...

{
  ...
  "candidates": [
    {
      "pathogen": "<scientific name as identified>",
  ...
  "in_mvp_database": <true if at least one candidate has a non-null mvp_match_name>,
  "biosecurity_alert": "<alert ID string if a Critical Alert Pathogen is detected, otherwise null>",
  "needs_more_info": <true if image quality is too poor or symptoms are too ambiguous>,
...
- ALWAYS end with: "Basado en estudios in vitro. Validar en campo antes de aplicar a escala."
